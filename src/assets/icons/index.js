import IcHomeOn from './HomeOn.svg';
import IcHomeOff from './HomeOff.svg';
import IcSettingOn from './SettingOn.svg';
import IcSettingOff from './SettingOff.svg';
import IcPlay from './Play.svg';
import IcBackOn from './BackOn.svg';
import IcBackOff from './BackOff.svg';
import IcSearch from './Search.svg';
import IcStar from './Star.svg';
import IcMonth from './bulan.svg';
import IcInfo from './Info.svg';
import IcMessage from './Message.svg';
import IcEdit from './Edit.svg';
import IcRate from './Rate.svg';

export {
  IcHomeOn, IcHomeOff, IcSettingOn, IcSettingOff, IcPlay, IcBackOn, IcBackOff, IcSearch, IcStar, IcMonth, IcInfo, IcMessage, IcEdit, IcRate,
};
